  ...
  if (periods.length === 0) {
    return (
      <div className="text-center py-20">
        ...
        <p className="text-base text-slate-500 mb-6">上传 TikTok 后台导出的 Excel 数据，开始追踪运营表现</p>
        <Link
          to="/upload"
  ...
  return (
    <div className="space-y-8">
      <div className="flex items-center justify-between">
        <div>
          <h2 className="text-3xl font-bold text-slate-900">数据总览</h2>
      ...
      {/* KPI Cards - cumulative totals */}
      <div className="grid grid-cols-2 lg:grid-cols-3 gap-5">
        <KpiCard title="累计 GMV" value={formatCurrency(total.gmv)} icon="💰" />
        <KpiCard title="累计 Orders" value={formatNumber(total.orders)} icon="📦" />
        <KpiCard title="整体 CTR" value={`${ctr.toFixed(2)}%`} icon="👆" />
        <KpiCard title="整体 CVR" value={`${cvr.toFixed(2)}%`} icon="🎯" />
        <KpiCard title="整体 AOV" value={formatCurrency(total.aov)} icon="🏷️" />
        <KpiCard title="累计 Customers" value={formatNumber(total.customers)} icon="👥" />
      </div>
      ...
      {/* CTR & CVR trends - by period type */}
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-5">
        <CtrCvrTrend data={monthlyCtrCvrData} title="月度 CTR & CVR 趋势" />
        <CtrCvrTrend data={weeklyCtrCvrData} title="周度 CTR & CVR 趋势（日数据按周聚合）" />
      </div>

      {/* Source & Funnel - aggregated across all periods */}
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-5">
        <div className="bg-white rounded-xl border border-slate-200 p-5">
          <h3 className="text-base font-semibold text-slate-700 mb-4">GMV 来源拆解（累计）</h3>
      ...
      <div className="bg-white rounded-xl border border-slate-200 overflow-hidden">
        <div className="px-5 py-4 bg-slate-50 border-b border-slate-100">
          <h3 className="text-base font-semibold text-slate-700">各周期概况</h3>
        </div>
        <div className="overflow-x-auto">
          <table className="w-full text-base">
            <thead>
              <tr className="border-b border-slate-100">
                <th className="text-left px-5 py-3 font-medium text-slate-500">周期</th>
                <th className="text-right px-5 py-3 font-medium text-slate-500">GMV</th>
                <th className="text-right px-5 py-3 font-medium text-slate-500">Orders</th>
                <th className="text-right px-5 py-3 font-medium text-slate-500">AOV</th>
                <th className="text-right px-5 py-3 font-medium text-slate-500">Customers</th>
                <th className="text-right px-5 py-3 font-medium text-slate-500">GMV环比</th>
              </tr>
            </thead>
            <tbody className="divide-y divide-slate-50">
              {periods.map((p) => (
                <tr key={p.id} className="hover:bg-slate-50/50">
                  ...
                  <td className="px-5 py-3 text-right font-medium text-slate-800">{formatCurrency(p.overview.gmv)}</td>
                  <td className="px-5 py-3 text-right text-slate-700">{formatNumber(p.overview.orders)}</td>
                  <td className="px-5 py-3 text-right text-slate-700">{formatCurrency(p.overview.aov)}</td>
                  <td className="px-5 py-3 text-right text-slate-700">{formatNumber(p.overview.customers)}</td>
                  <td className={`px-5 py-3 text-right font-medium ${
                    p.overviewChange.gmv > 0 ? 'text-emerald-600' : p.overviewChange.gmv < 0 ? 'text-red-500' : 'text-slate-400'
                  }`}>
              ...
              <tr className="bg-indigo-50/50 font-semibold">
                <td className="px-5 py-3 text-indigo-700">累计</td>
                <td className="px-5 py-3 text-right text-indigo-700">{formatCurrency(total.gmv)}</td>
                <td className="px-5 py-3 text-right text-indigo-700">{formatNumber(total.orders)}</td>
                <td className="px-5 py-3 text-right text-indigo-700">{formatCurrency(total.aov)}</td>
                <td className="px-5 py-3 text-right text-indigo-700">{formatNumber(total.customers)}</td>
                <td className="px-5 py-3 text-right text-indigo-700">-</td>
              </tr>
            </tbody>
          </table>
        </div>
...
function AggregateSourceChart({ total }: { total: MetricRow }) {
  ...
  if (sum === 0) {
    return <p className="text-base text-slate-400 text-center py-12">暂无来源数据</p>;
  }

  const items = [
    { label: '直播', value: liveGmv, pct: (liveGmv / sum) * 100, color: 'bg-indigo-500' },
